{
  ...
  config = {
    networking.startupOnlineCheckUrls = ["https://example.com" "https://github.com/NixOS/nixpkgs"];
    systemd.services.network-connected = {
      description = "Startup network connection tester";
      wantedBy = ["network-online.target"];
      after = ["network.target"];
      script = let
        urls = lib.strings.concatStringsSep " " (map (value: "\"${value}\"") config.networking.startupOnlineCheckUrls);
      in ''
        array=(${urls})
        for var in "''${array[@]}"
        do
